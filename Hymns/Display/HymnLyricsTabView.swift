 ...
 */
struct HymnLyricsTabView<TabType: HymnLyricsTabItem>: View {
    ...
    var body: some View {
            ...
            VStack(alignment: .center) {
                ...
                if self.currentTab.a11yLabel == Text("chords") {
                    WebView(url: self.viewModel.chordsUrl!).eraseToAnyView()
                }
                if self.currentTab.a11yLabel == Text("guitar") {
                    WebView(url: self.viewModel.guitarUrl!).eraseToAnyView()
                }
                if self.currentTab.a11yLabel == Text("piano") {
                    WebView(url: self.viewModel.pianoUrl!).eraseToAnyView()
                }
                if self.currentTab.a11yLabel != Text("lyrics") && self.currentTab.a11yLabel != Text("chords") && self.currentTab.a11yLabel != Text("guitar") && self.currentTab.a11yLabel != Text("piano") {
