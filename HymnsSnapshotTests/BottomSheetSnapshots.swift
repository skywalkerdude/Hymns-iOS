...
class BottomSheetSnapshots: XCTestCase {
    ...
    func test_shareSheet() {
        assertSnapshot(matching: ShareSheet(activityItems: ["share text"]), as: .image())
    }

    func test_tagSheet_noTags() {
        let viewModel = TagSheetViewModel(hymnToDisplay: cupOfChrist_identifier)
        assertSnapshot(matching: TagSheetView(viewModel: viewModel, sheet: .constant(.tags)), as: .image())
    }
    ...
    func test_songInfo_empty() {
        ...
        assertSnapshot(matching: SongInfoSheetView(viewModel: viewModel), as: .image())
    }
